...
func initialize():
	GlobalLogger.log_i(_pre_log + "Initializing entity datasource...")
	registry = GameManager.get_registry()
	_load_entities()

func _load_entities():
	GlobalLogger.log_i(_pre_log + "Loading entities...")
	var dir_access : DirAccess = DirAccess.open(entities_dir)
	if !dir_access:
		GlobalLogger.log_e(_pre_log + "Could not open the entities folder : "+ entities_dir)
		return
