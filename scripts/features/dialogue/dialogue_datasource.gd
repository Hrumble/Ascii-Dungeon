...
func initialize():
	GlobalLogger.log_i(_pre_log + "Initializing Dialogue Datasource...")
	load_dialogues()
...
func get_dialogue_by_name(entity_name : String, dialogue_name : String) -> Dialogue:
	if !dialogues.has(entity_name):
		GlobalLogger.log_e(_pre_log + "The dialogues for: " + entity_name + " does not exist! Are you sure you called DialogueDatasource.initialize()?")
		return null
	if !dialogues[entity_name].has(dialogue_name):
		GlobalLogger.log_e(_pre_log + "The dialogue called: " + dialogue_name + " does not exist!")
		return null
...
func load_dialogues():	
	GlobalLogger.log_i(_pre_log + "Loading dialogues...")
	var dir : DirAccess = DirAccess.open(dialogue_folder)
	if !dir:
		GlobalLogger.log_e(_pre_log + "Could not open the dialogue folder: " + dialogue_folder)
		return
