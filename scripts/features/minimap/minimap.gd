...
const ROOM_GAP : int = 2

func _draw():
	if _room_handler == null:
		return

	for room : Room in _room_handler.generated_rooms.values():
		_draw_room(room)

func _draw_room(room : Room):
	var translated_polygon : PackedVector2Array = []
	for point in room.polygon:
		translated_polygon.append(point + (room.position * (room.radius * ROOM_GAP)))
	draw_colored_polygon(translated_polygon, Color.BLACK)
	pass
...
func _on_room_generated(_room : Room):
	queue_redraw()
	pass

func _on_player_enter_room(_pos : Vector2i):
	queue_redraw()
	pass
