...
func add_move(move_id : String):
	if !GameManager.get_registry().has(move_id):
		GlobalLogger.log_e("CombatMoveContainer> cannot add move with id : %s because it does not exist in the registry" % move_id)
		return
...
func remove_move(move_id : String):
	if !has(move_id):
		GlobalLogger.log_w("CombatMoveContainer> attempted to remove a move that is not in the container: %s" % move_id)
		return
...
func get_ref(move_id : String) -> CombatMove:
	if !has(move_id):
		GlobalLogger.log_w("CombatMoveContainer> cannot get ref for %s because it is not available" % move_id)
		return null
