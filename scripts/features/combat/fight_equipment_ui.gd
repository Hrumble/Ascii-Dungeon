...
func _ready():
	...
	text_container.pivot_offset_ratio = Vector2(.5, .5)
	text_container.hide()
...
func step_up(time: float):
	await UIAnimations.displace(self, time, self.position - Vector2(0, 16), self.scale)
...
func shake_and_display_text(text: String, time: float, shake_speed_scale : float, texture: Texture2D = null):
	text_container.show()
